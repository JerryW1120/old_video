function [] = Demo_og(genarate_path, waiting_to_be_cut_dir, reference_dir, save_dir, rate)
...
for i = 1:size(filepaths_og)
    ...
    I2  = im2double(imread(fullfile(folder_og,filepaths_waiting_to_be_cut(i).name))); 

    s = rate;                                             % s = len1/len2, len1&lend2 are the focal length of captured image (len1>len2)
    r = 1 - 1/s;                                       % Scale
    ...
    imwrite(I2_zoom, [save_dir, filepaths_waiting_to_be_cut(i).name])
end
