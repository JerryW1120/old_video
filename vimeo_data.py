...
def split_fold(baseline, count, png_list):
    ...
    while 1:        
        if count >= len(all_file):#文件尾问题解决
            ...
            cmpr1 = plt.imread(os.path.join(input_dir, all_file[count - 1]))
            cmpr2 = plt.imread(os.path.join(input_dir, all_file[count]))
...
while count <= len(all_file):
    ...
    folder_dir = os.path.join(output_dir, str(img_list[0].split(".")[0]) + ' - ' + str(img_list[-1].split(".")[0]))
    os.makedirs(folder_dir)
    print("已创建文件夹%s" % (folder_dir))
            
    for file in answer[2]:
        copyfile(input_dir + '\\' + str(file), folder_dir + '\\' + str(file))
